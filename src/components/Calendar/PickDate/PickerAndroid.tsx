...
class PickerAndroidItem extends Component<Props, State> {
...
export default class PickerAndroid extends Component<Props, State> {
  ...
  _handlePanResponderMove(evt: any, gestureState: any) {
    ...
    if (dy > 0) {
      ...
      this._move(dy < (this.index - this.state.items.length + 1) *
      40 ? (this.index - this.state.items.length + 1) * 40 : dy);
    }
  }

  _handlePanResponderRelease(evt: any, gestureState: any) {
  ...
  _renderItems(items) {
    ...
    const upItems = []; const middleItems = []; const downItems = [];
